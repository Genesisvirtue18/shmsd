...
import { ROUTES, slugifySegment } from '@/lib/routes'
...
function normalizeDoctorSlug(slug: string) {
  return slug.replace(/^dr-/, '')
}
...
export function generateStaticParams() {
  return MANAGEMENT_TEAM.filter((member) => member.slug).map((member) => ({
    speciality: slugifySegment(member.department),
    doctor: `dr-${member.slug}`,
  }))
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { doctor } = await params
  const member = getMember(normalizeDoctorSlug(doctor))
  if (!member) return {}
...
export default async function DoctorDetailPage({ params }: { params: Promise<Params> }) {
  const { speciality, doctor } = await params
  const doctorSlug = normalizeDoctorSlug(doctor)
  const member = getMember(doctorSlug)
            ...
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <DetailBlock
                  ...
                  text={focusAreas[0]}
                  icon={isAshish ? ShieldCheck : HeartPulse}
                />
                <DetailBlock
                  title="Care style"
                  text={focusAreas[1]}
                  icon={isAshish ? BriefcaseMedical : Sparkles}
                />
              </div>
